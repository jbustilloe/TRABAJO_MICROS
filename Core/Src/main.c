...
// Rango de brillo del LED (0-255)
const uint32_t brightness_range = 255;
...
int main(void)
{
  ...
  // Iniciar conversión continua del ADC1
  HAL_ADC_Start_DMA(&hadc1, &potentiometer_value, 1);
  ...
  while (1)
  {
	    ...
	    // Ajustar el brillo del LED en función del valor del potenciómetro
	    uint32_t brightness = (potentiometer_value * brightness_range) / 4095; // Valor máximo de ADC (12 bits)
...
static void MX_GPIO_Init(void)
{
  ...
  //*Asignacion PIN 12 al canal1 del Timer2 utilizando funcion alternativa AF1
